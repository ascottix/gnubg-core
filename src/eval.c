...
EvalInitialise(char *szWeights, char *szWeightsBinary, int fNoBearoff, void (*pfProgress)(unsigned int))
{
    ...
    if (!fReadWeights) {
        g_printerr(_("GNU Backgammon couldn't find a weights file."));
        exit(EXIT_FAILURE);
    }
}
...
EvalKey(const evalcontext *pec, const int nPlies, const cubeinfo *pci, int fCubefulEquity)
{
    ...
    if (nPlies || fCubefulEquity) {
        ...
        if (pci->nMatchTo)
            ...
            iKey ^=
                ((pci->fCubeOwner < 0 ? 2 : pci->fCubeOwner == pci->fMove) << 23) ^ (pci->fJacoby << 26) ^ (pci->fBeavers << 27);
...
Utility(float ar[NUM_OUTPUTS], const cubeinfo *pci)
{
    ...
    if (!pci->nMatchTo) {
        ...
        return ar[OUTPUT_WIN] * 2.0f - 1.0f +
               (ar[OUTPUT_WINGAMMON] - ar[OUTPUT_LOSEGAMMON]) *
                   pci->arGammonPrice[0] +
        ...
        return ar[OUTPUT_WIN] * 2.0f - 1.0f + ar[OUTPUT_WINGAMMON] * pci->arGammonPrice[pci->fMove] - ar[OUTPUT_LOSEGAMMON] * pci->arGammonPrice[!pci->fMove] + ar[OUTPUT_WINBACKGAMMON] * pci->arGammonPrice[2 + pci->fMove] - ar[OUTPUT_LOSEBACKGAMMON] * pci->arGammonPrice[2 + !pci->fMove];
    }
}
                 ...
                 int iPip, int cPip, const TanBoard anBoard, int anMoves[], int fPartial)
{
    ...
    if (anBoard[1][24]) { /* on bar */
        ...
        if (GenerateMovesSub(pml, anRoll, nMoveDepth + 1, 23, cPip + anRoll[nMoveDepth], (ConstTanBoard)anBoardNew, anMoves, fPartial))
                 ...
                 const int fMove, const int nMatchTo, const int anScore[2], const int fCrawford, const bgvariation bgv)
{
...

    {
        ...
        if ((!nAway0 || !nAway1) && !fCrawford) {
            if (!nAway0)
                memcpy(pci->arGammonPrice, aaaafGammonPricesPostCrawford[LogCube(pci->nCube)][nAway1][0], 4 * sizeof(float));
            else
                memcpy(pci->arGammonPrice, aaaafGammonPricesPostCrawford[LogCube(pci->nCube)][nAway0][1], 4 * sizeof(float));
        } else
            memcpy(pci->arGammonPrice, aaaafGammonPrices[LogCube(pci->nCube)][nAway0][nAway1], 4 * sizeof(float));
    }
...
GetDPEq(int *pfCube, float *prDPEq, const cubeinfo *pci)
{
    ...
    if (!pci->nMatchTo) {
        ...
        fCube = (!pci->fCrawford) &&
                ...
                (!(fPostCrawford && (pci->anScore[pci->fMove] == pci->nMatchTo - 1))) && ((pci->fCubeOwner == -1) || (pci->fCubeOwner == pci->fMove));
...
Cl2CfMatchCentered(float arOutput[NUM_OUTPUTS], cubeinfo *pci, float rCubeX)
{
    ...
    if (arOutput[OUTPUT_WIN] <= rOppTG) {
        ...
        float rMWCLose = (1.0f - rG1 - rBG1) * aarMETResult[pci->fMove][NDL] + rG1 * aarMETResult[pci->fMove][NDLG] + rBG1 * aarMETResult[pci->fMove][NDLB];
        ...
        float rMWCWin = (1.0f - rG0 - rBG0) * aarMETResult[pci->fMove][NDW] + rG0 * aarMETResult[pci->fMove][NDWG] + rBG0 * aarMETResult[pci->fMove][NDWB];
    ...
    for (ici = 0, i = 0; ici < cci; ici++) {
        ...
        if (aciCubePos[ici].nCube > 0) {
            ...
            SetCubeInfo(&aci[i],
                        ...
                        fInvert ? !aciCubePos[ici].fMove : aciCubePos[ici].fMove,
...
getPercent(const cubedecision cd, const float arDouble[])
{
    ...
    case TOOGOODRE_PASS:
        ...
        if (arDouble[OUTPUT_NODOUBLE] > arDouble[OUTPUT_TAKE])
            ...
            return (arDouble[OUTPUT_NODOUBLE] - arDouble[OUTPUT_DROP]) / (arDouble[OUTPUT_TAKE] - arDouble[OUTPUT_DROP]);
...
ScoreMoves(movelist *pml, const cubeinfo *pci, const evalcontext *pec, int nPlies)
{
    ...
    for (i = 0; i < pml->cMoves; i++) {
        ...
        if ((pml->amMoves[i].rScore > pml->rBestScore) || ((pml->amMoves[i].rScore == pml->rBestScore) && (pml->amMoves[i].rScore2 >
...
extern int
FindnSaveBestMoves(movelist *pml, int nDice0, int nDice1, const TanBoard anBoard, positionkey *keyMove, const float rThr, const cubeinfo *pci, const evalcontext *pec,
                   movefilter aamf[MAX_FILTER_PLIES][MAX_FILTER_PLIES])
{
    ...
    mFilters = (pec->nPlies > 0 && pec->nPlies <= MAX_FILTER_PLIES) ? aamf[pec->nPlies - 1] : aamf[MAX_FILTER_PLIES - 1];
